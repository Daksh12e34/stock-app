...
With educational feedback for beginners.
...
st.markdown(
    ...
    .ps-teach-box p, .ps-teach-box li {
        color: #cbd5e1;
        font-size: 14px;
        line-height: 1.8;
    }
    .ps-teach-box ul { margin: 8px 0; padding-left: 20px; }
    ...
    .ps-lesson-card p, .ps-lesson-card li {
        color: #cbd5e1;
        font-size: 13.5px;
        line-height: 1.8;
    }
    .ps-lesson-card ul { margin: 8px 0; padding-left: 20px; }
    .ps-lesson-card b { color: #f8fafc; }

    .ps-tip-box {
        background: linear-gradient(135deg, rgba(250,204,21,0.10), rgba(245,158,11,0.05));
    ...
    .ps-warn-box {
        background: linear-gradient(135deg, rgba(239,68,68,0.10), rgba(153,27,27,0.05));
...
def get_user_score(username):
    data = load_scores()
    return data.get(username, {"score": 0, "games": 0, "correct": 0, "total": 0})


def add_game_result(username, correct, total):
    data = load_scores()
    user = data.get(username, {"score": 0, "games": 0, "correct": 0, "total": 0})
    user["score"] += correct
    user["games"] += 1
...
# ============================================================================
# QUIZ GAME — WITH EDUCATIONAL FEEDBACK
# ============================================================================
QUIZ_TICKERS = [
...
def generate_quiz_feedback(user_guess, current):
    ...
    # Personalized lesson based on mistake
    if guess == "BUY" and actual_dir == "DOWN":
        lesson = "You chose **BUY** but the price **went DOWN**. This is the most common beginner mistake — buying into a falling stock hoping it bounces. When you see **lower highs and lower lows**, that's the market telling you sellers are in control."
        warning = "**Pro tip:** Never BUY just because the price \"looks cheap\". A falling price can keep falling. Wait for the chart to show **higher lows** before entering."
    elif guess == "BUY" and actual_dir == "FLAT":
        lesson = "You chose **BUY** but the price **stayed flat**. In a sideways market, buyers and sellers are equally strong — no breakout happened. Save your money for a clearer signal."
        warning = "**Pro tip:** Sideways markets are the hardest to trade. If the price is bouncing in a $5 range for days, **HOLD** is the smart move."
    elif guess == "SELL" and actual_dir == "UP":
        lesson = "You chose **SELL** but the price **went UP**. You bet against an uptrend. When buyers are pushing the price to **higher highs**, selling is fighting the trend."
        warning = "**Pro tip:** \"The trend is your friend.\" If a stock is in a strong uptrend, don't bet against it — join it."
    elif guess == "SELL" and actual_dir == "FLAT":
        lesson = "You chose **SELL** but the price **stayed flat**. There was no downward pressure — just balance. HOLD was safer here."
        warning = "**Pro tip:** Selling in a quiet market usually means you're just paying commissions for nothing."
    elif guess == "HOLD" and actual_dir == "UP":
        lesson = "You chose **HOLD** but the price **broke out upward**. You missed a BUY opportunity! Strong momentum + rising volume often signals a breakout."
        warning = "**Pro tip:** When a stock sits quiet then suddenly moves up with **high volume**, that's a strong BUY signal."
    elif guess == "HOLD" and actual_dir == "DOWN":
        lesson = "You chose **HOLD** but the price **broke down**. Sometimes waiting costs money — when sellers take control, exiting early protects your capital."
        warning = "**Pro tip:** A sharp drop with **high volume** is a warning sign — SELL or at least reduce your position."
    elif guess == "BUY" and actual_dir == "UP":
        lesson = "**Perfect BUY!** You correctly read the upward momentum. You noticed the price was climbing and went with it. This is exactly how traders think — **ride the trend**."
        warning = "**Pro tip:** When a trade goes right, don't get greedy. Set a target and take profits when you hit it."
        ...
        warning = "**Pro tip:** In a downtrend, patience pays. Don't rush to buy the \"bottom\" — wait for clear reversal signals."
    else:
        lesson = "**Perfect HOLD!** You correctly identified that the market was undecided. In choppy, sideways markets, doing nothing is often the smartest move."
...
def render_quiz_feedback(guess, current, was_correct):
    """Display educational feedback box after the user's guess."""
    fb = generate_quiz_feedback(guess, current)

    # Big banner
    if was_correct:
        st.markdown('<div class="ps-feedback correct">✅ CORRECT!</div>', unsafe_allow_html=True)
    else:
        st.markdown('<div class="ps-feedback wrong">❌ WRONG</div>', unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        f'<b>📖 Lesson:</b> {fb["lesson"]}<br><br>'
        f'{fb["warning"]}'
        f'</div>',
        unsafe_allow_html=True,
    )

    # Chart reading mini-guide
        ...
        st.markdown(
            ...
            <div class="ps-lesson-card">
                ...
                <ul>
                    ...
                    <li>➖ <b>Thin body / doji</b> = small move, uncertainty</li>
                </ul>
            </div>
            """,
            unsafe_allow_html=True,
...
if search_results:
    ...
    st.sidebar.selectbox(
        ...
        match_labels, key="_search_pick_label", on_change=_apply_search_pick,
    )
...
auto_tune = st.sidebar.checkbox(
    "⚡ Auto-tune hyperparameters", value=False,
...
with tab_analysis:
    ...
    st.markdown(
        f'<div class="ps-sub">{ticker_symbol} · {start_date} → {end_date} · Priced in {display_currency}</div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '<div class="ps-sub">Model: Random Forest · Features: OHLCV + SMA50 + EMA20 + Lags + Volatility + RSI + MACD</div>',
        unsafe_allow_html=True,
    )
    ...
    if st.button("▶ Run AI Prediction"):
        ...
        if len(model_df) > 60:
            ...
            if auto_tune and len(X_train) > 80:
                with st.spinner("Auto-tuning..."):
                    ...
                    search = RandomizedSearchCV(
                        ...
                        n_iter=8, cv=TimeSeriesSplit(n_splits=3),
            ...
            with fcol2:
                st.markdown(
                    f'<div style="padding-top:22px;">AI Signal:<br>'
                    f'<span class="ps-signal {css_class}">{arrow} {signal}</span></div>',
                    unsafe_allow_html=True,
                )
            st.caption(
                f"**95% CI:** {fmt(ci_low, display_currency)} — {fmt(ci_high, display_currency)} "
                f"({len(final_model.estimators_)} trees)"
            )
            ...
            fi_fig = go.Figure(go.Bar(
                ...
                marker=dict(color=importances.values[::-1],
...
# ============================================================================
# TAB 2 — GAME (with educational feedback)
# ============================================================================
with tab_game:
    ...
    if st.session_state["logged_in_user"] is None:
        ...
        st.markdown(
            ...
            <div class="ps-player-card">
                ...
                <div style="flex:1;">
                    <div class="ps-player-name">{user}</div>
        ...
        with game_tab1:
            ...
            def _render_dots():
                ...
                for i in range(ROUNDS_PER_GAME):
                    if i < st.session_state["quiz_round"] - 1:
                        cls = "done"
            ...
            if st.session_state.get("quiz_finished"):
                ...
                if final_score == ROUNDS_PER_GAME:
                    st.markdown('<div class="ps-feedback correct">🏆 PERFECT!</div>', unsafe_allow_html=True)
                    ...
                    st.markdown('<div class="ps-feedback correct">👍 NICE!</div>', unsafe_allow_html=True)
                else:
                    st.markdown('<div class="ps-feedback wrong">💪 KEEP TRYING!</div>', unsafe_allow_html=True)

                st.markdown(
                    f'<div style="text-align:center;font-size:32px;font-weight:900;color:#f8fafc;margin:20px 0;">'
                    f'{final_score} / {ROUNDS_PER_GAME}</div>',
                    unsafe_allow_html=True,
                )
                st.balloons()
                ...
                _render_dots()
                st.markdown(
                    f'<div style="text-align:center;font-size:24px;font-weight:800;color:#f8fafc;margin:16px 0;">'
                    f'Round {st.session_state["quiz_round"]} of {ROUNDS_PER_GAME}</div>',
                    unsafe_allow_html=True,
                )
                st.markdown(
                    '<div style="text-align:center;color:#94a3b8;margin-bottom:16px;">'
                    'Look at the chart → guess BUY / SELL / HOLD</div>',
                    unsafe_allow_html=True,
                )
                ...
                if current is None:
                    ...
                    _render_dots()
                    st.markdown(
                        f'<div style="text-align:center;font-size:22px;font-weight:800;color:#f8fafc;">'
                        f'Round {st.session_state["quiz_round"]} / {ROUNDS_PER_GAME} '
                        f'&nbsp;·&nbsp; Score: {st.session_state["quiz_score"]}</div>',
                        unsafe_allow_html=True,
                    )

                    quiz_fig = go.Figure(go.Candlestick(
                        x=current["df"].index,
                    ...
                    if not st.session_state["quiz_revealed"]:
                        st.markdown(
                            '<div style="text-align:center;color:#cbd5e1;font-size:15px;'
                            'margin:14px 0 10px 0;font-weight:600;">'
                            '🎯 What will happen next?</div>',
                            unsafe_allow_html=True,
                        )
                        ...
                        with c1:
                            if st.button("📈 BUY", use_container_width=True, key="ans_buy"):
                                ...
                                st.session_state["quiz_last_guess"] = "BUY"
                                st.rerun()
                        with c2:
                            if st.button("📉 SELL", use_container_width=True, key="ans_sell"):
                                ...
                                st.session_state["quiz_revealed"] = True
                                st.session_state["quiz_last_correct"] = correct
                                st.session_state["quiz_last_guess"] = "SELL"
                                st.rerun()
                        with c3:
                            if st.button("✋ HOLD", use_container_width=True, key="ans_hold"):
                                ...
                                st.session_state["quiz_revealed"] = True
                                st.session_state["quiz_last_correct"] = correct
                                st.session_state["quiz_last_guess"] = "HOLD"
                                st.rerun()
                    else:
        ...
        with game_tab2:
            ...
            with st.expander("📖 Lesson 2 — What is Market Cap?"):
                st.markdown(
                    ...
                    <div class="ps-lesson-card">
                        ...
                        <p>💡 <b>Why it matters:</b> A ₹10 stock in a ₹1 lakh company is very different from a ₹10 stock in a ₹10,000 crore company. Market cap tells you the real size.</p>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
            ...
            with st.expander("📖 Lesson 3 — BUY / SELL / HOLD Signals"):
                st.markdown(
                    ...
                    <div class="ps-lesson-card">
                        ...
                        <ul>
                            <li><b>BUY</b> → you expect the price to rise &gt;+1%<br>
                                <i>Look for: higher highs, green candles stacking, rising volume</i></li>
                            <li><b>SELL</b> → you expect the price to fall &gt;-1%<br>
                                <i>Look for: lower lows, red candles stacking, panic selling</i></li>
                            <li><b>HOLD</b> → you expect it to stay flat (±1%)<br>
                                <i>Look for: sideways pattern, no clear direction</i></li>
                        </ul>
                    </div>
                    """,
                    unsafe_allow_html=True,
            ...
            with st.expander("📖 Lesson 4 — Reading Trends"):
                st.markdown(
                    ...
                    <div class="ps-lesson-card">
                        ...
                        <ul>
                            <li><b>Uptrend</b> — each peak is higher than the last, each dip is higher too. → BUY zone.</li>
                            <li><b>Downtrend</b> — each peak is lower, each dip is lower too. → SELL zone.</li>
                            <li><b>Sideways</b> — price bounces in a range. → HOLD zone.</li>
                        </ul>
                        <p>🔥 <b>Breakout</b>: when price suddenly exits the pattern with high volume → strongest signal.</p>
                        <p>🎯 <b>Golden rule:</b> "The trend is your friend." Trade WITH the trend, not against it.</p>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
            ...
            if not board:
                ...
                for i, (u, s) in enumerate(board, 1):
                    ...
                    st.markdown(
                        f'<div class="ps-lb-row {row_cls}">'
                        f'<span class="ps-lb-name">{medal} {u}</span>'
                        f'<span class="ps-lb-score">{s} pts</span>'
                        f'</div>',
                        unsafe_allow_html=True,
                    )
...
st.markdown(
    ...
    <div class="ps-footer">
        ...
        <div class="ps-footer-badges">
            ...
            <span>Quiz Game</span><span>Login System</span><span>Beginner Education</span>
        </div>
    </div>
    """,
    unsafe_allow_html=True,
