...
Single-file version for GitHub + Streamlit Cloud.
...
# ============================================================================
# CUSTOM CSS — FINTECH DARK THEME + GAME UI
# ============================================================================
st.markdown(
    """
    <style>
    ...
    [data-testid="stHorizontalBlock"]:has(> [data-testid="column"]:nth-child(3):last-child) > [data-testid="column"]:nth-child(1) button {
        ...
        box-shadow: 0 8px 24px rgba(34,197,94,0.35), 0 0 40px rgba(34,197,94,0.15) !important;
    ...
    [data-testid="stHorizontalBlock"]:has(> [data-testid="column"]:nth-child(3):last-child) > [data-testid="column"]:nth-child(2) button {
        ...
        box-shadow: 0 8px 24px rgba(239,68,68,0.35), 0 0 40px rgba(239,68,68,0.15) !important;
        font-size: 20px !important;
        font-weight: 900 !important;
        padding: 22px 12px !important;
        color: #fff !important;
    ...
    [data-testid="stHorizontalBlock"]:has(> [data-testid="column"]:nth-child(3):last-child) > [data-testid="column"]:nth-child(3) button {
        ...
        box-shadow: 0 8px 24px rgba(245,158,11,0.35), 0 0 40px rgba(245,158,11,0.15) !important;
        font-size: 20px !important;
        font-weight: 900 !important;
        padding: 22px 12px !important;
        color: #fff !important;
    ...
    if st.button("▶ Run AI Prediction"):
        ...
        if len(model_df) > 60:
            ...
            pred_df = pd.DataFrame({
                "Actual": y_test * fx_rate, "Predicted": predictions * fx_rate,
            }, index=pred_dates)
...
# ============================================================================
# TAB 2 — GAME (LOGIN + QUIZ + TRAINING + LEADERBOARD)
# ============================================================================
with tab_game:
    ...
    if st.session_state["logged_in_user"] is None:
                ...
                if current is None:
                    ...
                    if not st.session_state["quiz_revealed"]:
                        ...
                        st.markdown(
                            ...
                            f'<b>Next close:</b> {current["actual_next"]:.2f} '
                            f'({current["change_pct"]:+.2f}%) &nbsp;→&nbsp; '
                            f'<b>{current["actual_direction"]}</b></div>',
                            unsafe_allow_html=True,
                        )
        ...
        with game_tab2:
            ...
            with st.expander("📖 Lesson 1 — Candlesticks"):
                st.write(
                    "Each candle = 1 day. Shows 4 prices:\n"
            ...
            with st.expander("📖 Lesson 2 — BUY / SELL / HOLD"):
                st.write(
                    "- **BUY** → you think price will go UP (more than +1%)\n"
                    "- **SELL** → you think price will go DOWN (more than -1%)\n"
                    "- **HOLD** → you think it stays roughly flat (±1%)\n\n"
                    "In our quiz, guesses are based on the **next day's closing price**."
                )

            with st.expander("📖 Lesson 3 — Reading Trends"):
                st.write(
                    "- **Uptrend:** higher highs + higher lows → BUY signal\n"
                    "- **Downtrend:** lower highs + lower lows → SELL signal\n"
                    "- **Sideways:** bouncing in a range → HOLD signal"
                )

            with st.expander("📖 Lesson 4 — Volume"):
                st.write(
                    "Volume = number of shares traded that day.\n"
                    "- **High volume + price up** = strong buying, likely continues up\n"
                    "- **High volume + price down** = strong selling, likely continues down\n"
...
st.markdown(
    ...
    <div class="ps-footer">
        ...
        <div class="ps-footer-badges">
            ...
            <span>Quiz Game</span><span>Login System</span>
        </div>
    </div>
    """,
    unsafe_allow_html=True,
