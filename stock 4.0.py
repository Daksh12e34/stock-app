...
# Single source of truth for the active ticker. Search results and presets
# only ever write to this ONCE, at the moment the user picks them (via the
# callbacks below) — never on every script rerun — so manually typing any
...
def _apply_preset():
    choice = st.session_state.get("_preset_choice")
    if choice and choice != "Custom (type below)":
        st.session_state["ticker_symbol"] = choice
...
search_query = st.sidebar.text_input(
    ...
    help="Works for any company listed on Yahoo Finance — US, Indian, or otherwise. "
         ...
         "it accepts ANY valid yfinance symbol, not only search results or presets.",
)
...
st.sidebar.selectbox(
    ...
    key="_preset_choice", on_change=_apply_preset,
)

# Manual override — always free-form, accepts ANY yfinance-supported symbol.
# Bound to the same session_state key that the callbacks above write to.
