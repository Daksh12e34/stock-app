...
With per-user registration, persistent scores, animated emojis, streaks, combos.
"""
...
st.markdown(
    ...
    .ps-badge::before {
        ...
        background: #4ade80; animation: pulseDot 2s ease-in-out infinite;
    }
    ...
    div[data-testid="stMetric"] {
        ...
        border: 1px solid #232a37; padding: 18px 20px; border-radius: 14px;
    ...
    [data-testid="stHorizontalBlock"]:has(> [data-testid="column"]:nth-child(3):last-child) > [data-testid="column"] button:hover {
        transform: translateY(-6px) scale(1.03) !important; filter: brightness(1.15);
    }
    ...
    .ps-xp-wrap { background: #131722; border: 1px solid #232a37; border-radius: 999px; height: 14px; overflow: hidden; margin: 10px 0; position: relative; }
    .ps-xp-fill { height: 100%; background: linear-gradient(90deg, #3b82f6, #a855f7, #22c55e); background-size: 200% 100%; border-radius: 999px; transition: width 0.8s cubic-bezier(0.4, 0, 0.2, 1); animation: gradientShift 4s linear infinite; box-shadow: 0 0 12px rgba(168,85,247,0.6); }
    .ps-xp-label { display: flex; justify-content: space-between; font-size: 11px; color: #94a3b8; font-family: 'JetBrains Mono', monospace; margin-bottom: 4px; }

    .ps-hud-row { display: flex; gap: 12px; margin: 14px 0; flex-wrap: wrap; }
    .ps-hud-chip { display: inline-flex; align-items: center; gap: 8px; padding: 8px 16px; border-radius: 999px; background: linear-gradient(135deg, rgba(19,23,34,0.9), rgba(15,18,26,0.9)); border: 1px solid #232a37; font-weight: 700; font-size: 14px; font-family: 'JetBrains Mono', monospace; }
    ...
    .ps-round-dot { width: 20px; height: 20px; border-radius: 50%; background: #1f2530; border: 2px solid #2f3947; transition: all 0.4s ease; }
    ...
    .ps-feedback { text-align: center; padding: 28px; border-radius: 18px; font-size: 42px; font-weight: 900; letter-spacing: 2px; animation: bounceIn 0.6s ease; margin: 16px 0; position: relative; }
    .ps-feedback.correct { background: linear-gradient(135deg, rgba(34,197,94,0.2), rgba(22,163,74,0.15)); border: 2px solid rgba(74,222,128,0.5); color: #4ade80; box-shadow: 0 0 40px rgba(34,197,94,0.3); }
    .ps-feedback.wrong { background: linear-gradient(135deg, rgba(239,68,68,0.2), rgba(153,27,27,0.15)); border: 2px solid rgba(248,113,113,0.5); color: #f87171; box-shadow: 0 0 40px rgba(239,68,68,0.3); animation: shake 0.6s ease; }

    .ps-emoji-container { position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); pointer-events: none; z-index: 9999; }
    .ps-floating-emoji { position: absolute; font-size: 60px; animation: floatUp 2s ease-out forwards; filter: drop-shadow(0 0 20px rgba(255,255,255,0.5)); }
    .ps-reaction-emoji { display: inline-block; font-size: 48px; animation: emojiPop 0.6s ease; filter: drop-shadow(0 0 15px rgba(255,255,255,0.4)); }

    .ps-result-card { background: linear-gradient(135deg, rgba(19,23,34,0.9), rgba(15,18,26,0.9)); border: 1px solid #232a37; border-radius: 14px; padding: 18px 22px; margin: 12px 0; font-family: 'JetBrains Mono', monospace; font-size: 14px; color: #cbd5e1; }
    .ps-result-card b { color: #f8fafc; }

    .ps-teach-box { background: linear-gradient(135deg, rgba(59,130,246,0.10), rgba(168,85,247,0.06)); border: 1px solid rgba(59,130,246,0.35); border-radius: 16px; padding: 22px 26px; margin: 16px 0; animation: fadeInUp 0.5s ease; }
    ...
    .ps-lesson-card { background: linear-gradient(135deg, rgba(19,23,34,0.85), rgba(15,18,26,0.85)); border: 1px solid rgba(35,42,55,0.8); border-radius: 14px; padding: 20px 24px; margin: 14px 0; }
    ...
    .ps-achievement { display: inline-flex; align-items: center; gap: 6px; padding: 8px 14px; border-radius: 10px; background: linear-gradient(135deg, rgba(250,204,21,0.15), rgba(245,158,11,0.08)); border: 1px solid rgba(250,204,21,0.4); font-size: 12.5px; font-weight: 700; color: #fde047; animation: bounceIn 0.5s ease; }

    .ps-hint-box { background: linear-gradient(135deg, rgba(250,204,21,0.12), rgba(245,158,11,0.06)); border: 1px solid rgba(250,204,21,0.4); border-radius: 12px; padding: 16px 20px; margin: 12px 0; color: #fef3c7; font-size: 14px; line-height: 1.7; animation: slideInLeft 0.5s ease; }
    ...
    .ps-auth-card {
        background: linear-gradient(135deg, rgba(19,23,34,0.95), rgba(15,18,26,0.95));
        border: 1px solid #232a37;
        border-radius: 18px;
        padding: 32px 36px;
        max-width: 520px;
...
# Reserved demo accounts — in-memory only, never saved to users.json
DEMO_USERS = {
    "demo": "demo123",
    "admin": "admin123",
    "player1": "pass1",
...
# ============================================================================
# AUTHENTICATION — with password hashing + registration
# ============================================================================
...
def save_users(data: dict):
    """Persist registered users to users.json."""
    try:
...
def verify_login(username: str, password: str) -> bool:
    """Check credentials. Demo accounts OR registered users."""
    username = (username or "").strip().lower()
...
# ============================================================================
# SCORE STORAGE (per-user, persistent)
# ============================================================================
...
def get_user_score(username: str) -> dict:
    data = load_scores()
    return data.get(username, {"score": 0, "games": 0, "correct": 0, "total": 0, "best_streak": 0})


def add_game_result(username: str, correct: int, total: int, best_streak: int = 0):
    data = load_scores()
...
if search_results:
    ...
    st.sidebar.selectbox("Matching results — select one", match_labels,
                         key="_search_pick_label", on_change=_apply_search_pick)
...
with tab_analysis:
    ...
    st.markdown(f'<div class="ps-sub">{ticker_symbol} · {start_date} → {end_date} · Priced in {display_currency}</div>',
                unsafe_allow_html=True)
    ...
    fig.add_trace(go.Candlestick(x=disp.index, open=disp["Open"], high=disp["High"],
        low=disp["Low"], close=disp["Close"], name="Price",
    ...
    st.markdown('<div class="ps-sub">Model: Random Forest · Features: OHLCV + SMA50 + EMA20 + Lags + Volatility + RSI + MACD</div>',
                unsafe_allow_html=True)
...
with tab_game:
    ...
    if st.session_state["logged_in_user"] is None:
        st.markdown('<div class="ps-auth-card">', unsafe_allow_html=True)
        st.markdown('<div class="ps-auth-title">🎮 Player Access</div>', unsafe_allow_html=True)
        st.markdown('<div class="ps-auth-sub">Log in to continue your journey — or create a fresh account.</div>',
                    unsafe_allow_html=True)
        ...
        with login_tab:
            ...
            with col1:
                ...
                if st.button("🎮 Log In", use_container_width=True, key="_login_btn"):
            ...
            with col2:
                st.markdown("""
                    **Demo accounts (for quick testing):**
                    ...
                    ⚠️ Demo accounts are shared — **register your own** for a private, persistent score.
                    """)

        with register_tab:
            col1, col2 = st.columns([1, 1])
            with col1:
                new_user = st.text_input("Choose Username", key="_reg_user",
                    help="Min 3 characters. Letters, numbers, _ and - only.")
                new_pass = st.text_input("Choose Password", type="password", key="_reg_pass",
                    help="Min 4 characters.")
                new_pass2 = st.text_input("Confirm Password", type="password", key="_reg_pass2")
                if st.button("📝 Create Account", use_container_width=True, key="_reg_btn"):
                    ok, msg = register_user(new_user, new_pass, new_pass2)
                    if ok:
                        st.success(f"✅ {msg}")
                        st.info(f"You can now log in as **{new_user.strip().lower()}**.")
                    else:
                        st.error(f"❌ {msg}")
            with col2:
                st.markdown("""
                    ...
                    - **Unique username** → your own private score
                    - **Persistent across devices** → log in anywhere, see your stats
                    - **Leaderboard entry** → compete with others
                    - **Password hashed** → never stored in plain text
                    
                    ### 💡 Tips
                    - Username: `daksh_01`, `trader_king`, `student7`
                    - Password: something you remember, min 4 chars
                    """)
    else:
        ...
        with game_tab2:
            ...
            with st.expander("📖 Lesson 2 — What is Market Cap?"):
                st.markdown("""
                    <div class="ps-lesson-card">
                        ...
                        <ul>
                            <li>🐘 <b>Large Cap (&gt;$10B)</b> — Apple, Microsoft, Reliance.</li>
                            <li>🐕 <b>Mid Cap ($2B–$10B)</b> — growing companies.</li>
            ...
            if not board:
                ...
                for i, (u, s) in enumerate(board, 1):
                    ...
                    highlight = " ← **you**" if u == user else ""
                    st.markdown(f'<div class="ps-lb-row {row_cls}">'
                                f'<span class="ps-lb-name">{medal} {u}{highlight}</span>'
...
st.markdown(
    ...
    <div class="ps-footer">
        ...
        <div class="ps-footer-badges">
            ...
            <span>Quiz Game</span><span>User Accounts</span><span>SHA-256 Auth</span>
            <span>Persistent Scores</span><span>Animated Emojis</span>
        </div>
    </div>
    """,
    unsafe_allow_html=True,
