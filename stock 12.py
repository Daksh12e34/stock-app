...
# ============================================================================
# CUSTOM CSS — FULL FINTECH + GAME THEME
# ============================================================================
st.markdown(
    """
    <style>
    ...
    @keyframes scorePop {
        0% { transform: scale(1); }
        50% { transform: scale(1.4); color: #facc15; text-shadow: 0 0 30px #facc15; }
    ...
    [data-testid="stHorizontalBlock"]:has(> [data-testid="column"]:nth-child(3):last-child) > [data-testid="column"]:nth-child(1) button {
        ...
        box-shadow: 0 8px 24px rgba(34,197,94,0.35), 0 0 30px rgba(34,197,94,0.15) !important;
    ...
    [data-testid="stHorizontalBlock"]:has(> [data-testid="column"]:nth-child(3):last-child) > [data-testid="column"]:nth-child(2) button {
        ...
        box-shadow: 0 8px 24px rgba(239,68,68,0.35), 0 0 30px rgba(239,68,68,0.15) !important;
        font-size: 20px !important; font-weight: 900 !important;
        padding: 22px 12px !important; color: #fff !important;
        letter-spacing: 1.5px !important; text-transform: uppercase !important;
        border-radius: 14px !important;
    ...
    [data-testid="stHorizontalBlock"]:has(> [data-testid="column"]:nth-child(3):last-child) > [data-testid="column"]:nth-child(3) button {
        ...
        box-shadow: 0 8px 24px rgba(245,158,11,0.35), 0 0 30px rgba(245,158,11,0.15) !important;
        font-size: 20px !important; font-weight: 900 !important;
        padding: 22px 12px !important; color: #fff !important;
        letter-spacing: 1.5px !important; text-transform: uppercase !important;
        border-radius: 14px !important;
    ...
    .ps-score-chip.pop { animation: scorePop 0.7s ease; }
    ...
    .ps-lb-row {
        display: flex; justify-content: space-between; align-items: center;
        padding: 12px 18px;
        background: linear-gradient(135deg, rgba(19,23,34,0.7), rgba(15,18,26,0.7));
        border: 1px solid #232a37;
        border-radius: 10px;
        margin-bottom: 8px;
...
def generate_hint(current):
    """Generate a helpful hint based on the chart pattern."""
    chart_df = current["df"]
...
def generate_quiz_feedback(user_guess, current):
    """Build rich educational feedback."""
    ...
    if guess == "BUY" and actual_dir == "DOWN":
        ...
        lesson = "You chose **BUY** but the price **stayed flat**. In a sideways market, buyers and sellers are equally strong — no breakout happened."
        warning = "**Pro tip:** Sideways markets are the hardest to trade. If the price is bouncing in a range for days, **HOLD** is the smart move."
        ...
        lesson = "**Perfect HOLD!** You correctly identified that the market was undecided. In choppy markets, doing nothing is often the smartest move."
...
def render_quiz_feedback(guess, current, was_correct, combo=1):
    """Display rich educational feedback."""
    fb = generate_quiz_feedback(guess, current)

    # Big banner with reaction emoji
    if was_correct:
        reaction = random.choice(["🎉", "🔥", "💪", "⚡", "🏆"])
        st.markdown(
            f'<div class="ps-feedback correct">'
            f'<span class="ps-reaction-emoji">{reaction}</span> CORRECT!'
            f'</div>',
            unsafe_allow_html=True,
        )
    else:
        reaction = random.choice(["😢", "😞", "💔", "😬"])
        st.markdown(
            f'<div class="ps-feedback wrong">'
            f'<span class="ps-reaction-emoji">{reaction}</span> WRONG'
            f'</div>',
            unsafe_allow_html=True,
        )
...
def render_floating_emoji(emoji, position="center"):
    """Render a floating emoji animation."""
    st.markdown(
...
def render_confetti():
    """Render falling confetti animation."""
    emojis = ["🎉", "✨", "🎊", "⭐", "💫", "🌟"]
    html = ""
    for i in range(15):
...
# ============================================================================
# TAB 2 — GAME (Enhanced with emojis, streaks, combos, achievements)
# ============================================================================
with tab_game:
    ...
    if st.session_state["logged_in_user"] is None:
        ...
        with game_tab1:
            ...
            if "quiz_round" not in st.session_state:
                ...
                st.session_state["quiz_score_changed"] = False
            ...
            def _render_hud():
                ...
                hint_text = "💡 Hint used" if hint_used else "💡 Hint available"

                st.markdown(
                    f'<div class="ps-hud-row">'
            ...
            # Show floating emoji if triggered
            ...
            if st.session_state.get("quiz_finished"):
                ...
                if achievements:
                    ...
                    badges_html = "".join(
                        f'<div class="ps-achievement">{a}</div>' for a in achievements
                    )
                ...
                if current is None:
                    ...
                    if not st.session_state["quiz_revealed"]:
                        ...
                        if st.session_state.get("quiz_hint_shown", False):
                            ...
                            st.markdown(
                                f'<div class="ps-hint-box"><b>💡 Hint:</b> {hint}</div>',
                                unsafe_allow_html=True,
                            )
                        ...
                        st.markdown(
                            ...
                            '<span class="ps-reaction-emoji">🤔</span></div>',
                            unsafe_allow_html=True,
                        )
                        st.markdown(
                            '<div style="text-align:center;color:#cbd5e1;font-size:15px;'
                            'font-weight:600;">🎯 What will happen next?</div>',
                            unsafe_allow_html=True,
                        )

                        # Hint button
                        if not st.session_state.get("quiz_hint_used", False):
                            if st.button("💡 Use Hint (costs 1 point)", use_container_width=False):
                                ...
                                st.rerun()
                        elif st.session_state.get("quiz_hint_shown", False):
                            pass
                        else:
...
# Helper for handling answers (defined at module level to be callable from within `with tab_game`)
def _handle_answer(correct, guess, user):
    ...
    if correct:
        ...
        st.session_state["quiz_trigger_emoji"] = random.choice(["😢", "💔", "😞"])
        # If no lives left → end game immediately
...
# Redefine the game tab's training section (was cut mid-flow above for clarity)
with tab_game:
    if st.session_state.get("logged_in_user"):
        user = st.session_state["logged_in_user"]
        score_info = get_user_score(user)

        # Find the training tab reference by re-creating it
        # (This appends a second set of tabs — but since we're inside `tab_game`,
        # we render training content in a separate expander for simplicity)
        st.markdown("---")
        st.markdown("### 📚 Training Lessons")
