...
def search_stocks(query: str):
    ...
    (symbol, display_label) tuples. This is what lets a user type
    'Amazon' or 'Infosys' and get back the correct ticker (AMZN,
    INFY.NS) instead of having to already know the exact symbol/suffix.
    """
    query = (query or "").strip()
    if len(query) < 1:
        return []

    # --- Primary method: Yahoo Finance's own autocomplete/search API ---
    try:
        resp = requests.get(
        ...
        for item in data.get("quotes", []):
            ...
            if exch:
                label_bits.append(f"({exch})")
            results.append((symbol, " ".join(label_bits), kind))
        # Prefer actual equities over options/futures/indices when available
        equities = [r for r in results if r[2] in ("EQUITY", "ETF", "")]
        ordered = equities + [r for r in results if r not in equities]
        return [(sym, label) for sym, label, _ in ordered][:8]
    except Exception:
        pass

    # --- Fallback method: yfinance's built-in Search (if available in this version) ---
    try:
        ...
        for item in getattr(s, "quotes", []):
            ...
            name = item.get("shortname") or item.get("longname") or ""
            exch = item.get("exchange", "")
            label = f"{symbol}" + (f" — {name}" if name else "") + (f" ({exch})" if exch else "")
...
# Keep the selected ticker in session_state so search results and presets
# can both push a value into the same text input.
...
search_query = st.sidebar.text_input(
    ...
    help="Works for any company listed on Yahoo Finance — US, Indian, or otherwise. "
         "Type a company name if you don't know the exact ticker symbol.",
)
...
if search_results:
    ...
    label_to_symbol = {label: sym for sym, label in search_results}
    picked_label = st.sidebar.selectbox("Matching results — select one", match_labels)
    st.session_state["ticker_symbol"] = label_to_symbol[picked_label]
elif search_query.strip():
    st.sidebar.caption("No matches found. Try a different spelling, or type the exact ticker below.")
...
if preset_choice != "Custom (type below)" and not search_query.strip():
    st.session_state["ticker_symbol"] = preset_choice

# Manual override always available — bound to the same session_state key,
# so search results / presets update it, and manual typing updates it too.
...
if df.empty or "Close" not in df.columns:
    st.error(
        ...
        "Use the search box above to confirm the exact ticker, or choose a different timeframe."
    )
    st.stop()
