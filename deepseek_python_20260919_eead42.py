...
Single-file version for GitHub deployment.
...
# ============================================================================
# CUSTOM CSS — FINTECH DARK THEME (RGB GLOW + SMOOTH)
# ============================================================================
st.markdown(
    """
    <style>
...
with tab_game:
    ...
    if st.session_state["logged_in_user"] is None:
        ...
        with col1:
            st.markdown(
                f'<div class="ps-section-title">🎮 Welcome, {user}!</div>',
                unsafe_allow_html=True,
            )
            st.markdown(
                f'<div class="ps-sub">Score: **{score_info["score"]}** · '
                f'Games: {score_info["games"]} · '
                f'Accuracy: {(score_info["correct"]/score_info["total"]*100) if score_info["total"] else 0:.0f}%</div>',
                unsafe_allow_html=True,
            )
        with col2:
        ...
        with game_tab1:
            ...
            if st.session_state.get("quiz_finished"):
                st.success(f"🏁 Game over! You scored **{st.session_state['quiz_score']} / {ROUNDS_PER_GAME}**")
            ...
            # Waiting for next chart
                ...
                st.info(f"**Round {st.session_state['quiz_round']} of {ROUNDS_PER_GAME}** — guess BUY / SELL / HOLD")
            ...
            # Show current round
                ...
                if current is None:
                    ...
                    quiz_fig.update_layout(
                        ...
                        title=f"{current['ticker']} — Last {CHART_DAYS} Days",
                    ...
                    if not st.session_state["quiz_revealed"]:
                        ...
                        st.markdown(
                            f"**Actual:** Next close was **{current['actual_next']:.2f}** "
                            f"({current['change_pct']:+.2f}%) → **{current['actual_direction']}**"
                        )
        ...
        with game_tab2:
            ...
            st.markdown('<div class="ps-sub">Learn before you play.</div>', unsafe_allow_html=True)
            ...
            with st.expander("📖 Lesson 2 — BUY / SELL / HOLD"):
                st.write(
                    "- **BUY** → you think price will go UP (>1%)\n"
                    "- **SELL** → you think price will go DOWN (>1%)\n"
                    "- **HOLD** → you think it stays flat (±1%)\n\n"
                    "In our quiz, guesses are based on **next day's closing price**."
                )
            ...
            with st.expander("📖 Lesson 3 — Reading Trends"):
                st.write(
                    ...
                    "- **Sideways:** bounces in a range → HOLD"
                )
            ...
            if not board:
                ...
                for i, (u, s) in enumerate(board, 1):
                    ...
                    st.write(f"{medal} **{u}** — {s} points")

            st.markdown("---")
            st.markdown("### 📊 Your Stats")
            st.write(f"- **Games played:** {score_info['games']}")
            st.write(f"- **Total correct:** {score_info['correct']}")
            st.write(f"- **Total questions:** {score_info['total']}")
            st.write(f"- **Overall score:** {score_info['score']}")
